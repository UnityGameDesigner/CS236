...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            try:
                directory_path = join(args.data_root, vidname)
                # List all wav files in the directory
                wav_files = [f for f in os.listdir(directory_path) if f.endswith('.wav')]
                # Check if there are any wav files
                if wav_files:
                    # Select the first wav file (or modify as needed to select a specific file)
                    wavpath = join(directory_path, wav_files[0])
...
if __name__ == "__main__":
    ...
    train_size = int(0.50 * len(full_dataset))
